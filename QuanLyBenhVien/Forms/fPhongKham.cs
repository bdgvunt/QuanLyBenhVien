...
{
    public partial class fPhongKham : Form
    {
        public fPhongKham()
        {
            InitializeComponent();
        ...
        private void LoadDS()
        {
            dgv.DataSource = PhongKhamCommon.Select();
                ...
                if (dgv.CurrentRow != null)
                    if (dgv.CurrentRow.Cells["cId"].Value.ToString() == "")  // chua co gia tri Id tuc la dong moi
                    {
                        if (TenCot == "cTen")
                        {
                            ...
                            PhongKhamCommon.Insert(dgv.CurrentRow.Cells["cTen"].Value.ToString(), "",  1, 1);
                            LoadDS();
                        }
                    }
                    else  // voi dong cu thi update thong tin
                    {
                        if (TenCot == "cTen")
                        {
                            PhongKham HT = new PhongKham((decimal)dgv.CurrentRow.Cells["cId"].Value);
                            HT.TenPhongKham = dgv.CurrentRow.Cells["cTen"].Value.ToString();
                        }
                        if (TenCot == "cGhiChu")
                        {
                            PhongKham HT = new PhongKham((decimal)dgv.CurrentRow.Cells["cId"].Value);
